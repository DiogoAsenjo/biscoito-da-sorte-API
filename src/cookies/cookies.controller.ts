...
export class CookiesController {
  ...
  async adicionarCookiePositivo(
    ...
    if (!body.mensagem) {
      res
        ...
        .send('É obrigatório escrever uma mensagem!');
      return;
    }
  ...
  async alterarCookie(
    @Res() res: Response,
    @Body() body: { id: number; mensagem: string },
  ): Promise<void> {
    if (!body.id || !body.mensagem) {
      res
        .status(HttpStatus.INTERNAL_SERVER_ERROR)
        .send('Os campos id e mensagem são obrigatórios!');
      return;
    }
  ...
  async deletarCookie(@Res() res: Response, @Body() body: { id: number }) {
    if (!body.id) {
      res
        .status(HttpStatus.BAD_REQUEST)
        .send(
          'É obrigatório digitar o id do cookie que pretende excluir, para visualizar todos acesse /cookies/mostrar-todos',
        );
      return;
    }
